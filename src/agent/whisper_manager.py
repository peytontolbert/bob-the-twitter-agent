...
class WhisperManager:
    def __init__(self, threshold=0.03, input_device=None):
        try:
            ...
            self.input_device = input_device  # This should be CABLE Output
    ...
    def start_listening(self, sample_rate=None, channels=2):
        ...
        try:
            ...
            # Query device info for input (CABLE Output)
    ...
    def is_speech_detected(self):
        ...
        try:
            ...
            if len(self.buffer) > 0:
                recent_audio = np.array(self.buffer[-int(0.1 * self.sample_rate):])  # Last 100ms
            ...
            logging.error(f"Error checking speech detection: {e}")
            return False
    ...
    def get_current_audio_level(self):
        ...
        try:
            if len(self.buffer) > 0:
                recent_audio = np.array(self.buffer[-int(0.1 * self.sample_rate):])
                if len(recent_audio) > 0:
                    return np.sqrt(np.mean(recent_audio**2))
            return 0.0
        
        except Exception as e:
            logging.error(f"Error getting audio level: {e}")
            return 0.0
